...
export function NovoEventoForm({ organizerId }: { organizerId: string }) {
  ...
  const [batches, setBatches] = useState<BatchEntry[]>([emptyBatch()]);
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const res = await fetch("/api/painel/eventos", {
        method: "POST",
    ...
    <main className="min-h-screen bg-gray-950 text-white">
      ...
      <header className="border-b border-gray-800 bg-gray-900/80 backdrop-blur-sm sticky top-0 z-10">
        <div className="max-w-3xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <h1 className="text-base font-semibold">Novo Evento</h1>
        ...
        <div className="flex items-center justify-between mt-10 pt-6 border-t border-gray-800">
          ...
          {isLastStep ? (
            ...
            >
              ...
              {isLoading ? "Salvando..." : "Publicar como Rascunho"}
            </button>
          ) : (
            <button
              onClick={() => setStep((s) => s + 1)}
